...
def main(computer: Computer, args: list, pipe: bool) -> SysCallStatus:
    ...
    if not find_file_result.success:
        if decode:
            try:
                output_text = f"{b64decode(' '.join(args)).decode()}"
            except Error:
                output_text = f"{__COMMAND__}: invalid input"
        else:
            output_text = f"{b64encode(' '.join(args).encode()).decode()}"
    else:
        output_text = ""
        for arg in args:
            find_file_result = computer.fs.find(args[0])
            if not find_file_result.success:
                ...
                if file.is_directory():
                    ...
                    # We need read perms!
                    if not file.check_perm("read", computer.get_uid(), computer).success:
                        output_text += f"{__COMMAND__}: {arg}: Permission denied\n"
                    else:
                        if decode:
                            try:
                                output_text += f"{b64decode(file.content).decode()}\n"
                            except Error:
                                output_text += f"{__COMMAND__}: {arg}: invalid input\n"
                        else:
                            output_text += f"{b64encode(file.content.encode()).decode()}\n"
