...
__DESCRIPTION__ = "compute and check SHA512 message digest"
__DESCRIPTION_LONG__ = "Print or check SHA512 (512-bit) checksums."
__VERSION__ = "1.2"
            ...
            if not find_file_result.success:
                ...
                if file.is_directory():
                    ...
                    if not file.check_perm("read", computer).success:
                        ...
                        hash = sha512(to_hash.encode()).hexdigest()
                        if args.tag:
                            output_text += f"SHA512 ({filename}) = {hash}\n"
                        else:
