...
def main(args: list, pipe: bool) -> Result:
    ...
    if not args:
        ...
        if not result.success:
            ...
            if result.message == ResultMessages.NOT_ALLOWED:
                return output(f"{__COMMAND__}: permission denied: {args.directory}", pipe, success=False,
                              success_message=ResultMessages.NOT_ALLOWED)

            if result.message == ResultMessages.IS_FILE:
